...
SYSTEM_PROMPT = (
    "You are the Communications Agent for OlympusOS broadcasting public safety messages in Milan. "
    "Output JSON only: "
    "{\"message_en\": \"English message\", \"message_it\": \"Italian message\", "
    "\"channel\": \"app|screen|speaker\", \"urgency\": \"info|warning|alert|emergency\", "
    "\"action_required\": \"what people should do\"}"
)
...
class CommunicationsAgent(BaseAgent):
    ...
    def _report(self, result: dict[str, Any], command: dict[str, Any]) -> None:
        """Write the broadcast message to agent_reports."""
        report = {"agent": self.name, "broadcast": result, "source_command": command}
        self.redis.rpush("agent_reports", json.dumps(report))
        logger.info(
            "[%s] Broadcast → urgency=%s channel=%s",
            self.name,
            result.get("urgency"),
            result.get("channel"),
        )
    ...
    def run(self) -> None:
        logger.info("[%s] Communications agent started (interval: %ds)", self.name, LOOP_INTERVAL)
        self.running = True
            ...
            try:
                ...
                for cmd in commands:
                    ...
                    if result:
                        ...
                        self.write_status({"last_urgency": result.get("urgency"), "last_channel": result.get("channel")})
        ...
        logger.info("[%s] Communications agent stopped", self.name)
